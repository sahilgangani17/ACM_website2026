...
export const NavbarHeader: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 pointer-events-none p-3 sm:p-4 md:p-6 flex items-center justify-between gap-2">
      {/* Brand Identity & Breadcrumb */}
      <div className="pointer-events-auto flex items-center gap-2 sm:gap-3 glass-panel px-3 sm:px-3.5 py-1.5 rounded-xl shrink-0">
        <div className="w-7 h-7 sm:w-6 sm:h-6 rounded-md overflow-hidden bg-white/95 p-0.5 flex items-center justify-center shrink-0 shadow-sm">
        ...
        <span className="font-heading font-black text-base tracking-wider text-white">
          ACM <span className="text-cyan-400 font-mono font-medium text-[10px] sm:text-xs ml-1 hidden sm:inline">BOULEVARD v1.0</span>
        </span>
        {/* Return to Globe Button */}
        <button
          onClick={triggerWarpToGlobe}
          className="ml-1.5 sm:ml-2 pl-2 sm:pl-3 border-l border-cyan-500/30 text-xs font-mono text-cyan-300 hover:text-white flex items-center gap-1.5 transition-colors"
      ...
      <div className="pointer-events-auto flex items-center gap-1.5 sm:gap-3">
        {/* State Indicator — hidden on small screens */}
        <div className="hidden lg:flex items-center gap-2 glass-panel px-3 py-1.5 rounded-lg text-xs font-mono text-slate-300">
          <Compass className="w-3.5 h-3.5 text-cyan-400" />
          <span className="text-cyan-400 font-bold">{cityMode}</span>
        </div>

        {/* Quality Tier Selector */}
        <div className="flex items-center gap-1 sm:gap-1 glass-panel p-1 rounded-lg text-xs font-mono">
        ...
        {/* Debug Toggle Button — icon-only on mobile */}
        <button
        ...
        {reducedMotion && (
          <div className="glass-panel p-1.5 sm:p-2 rounded-lg text-xs text-amber-400 flex items-center gap-1" title="Reduced Motion Active">
